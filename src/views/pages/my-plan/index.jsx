        ...
        if (paymentDetails?.paymentKeys?.name.match(/razorpay/i)) {
            ...
            const options = {
                ...
                handler: async function (response) {
                    ...
                    const postData = { orderCreationId, razorpayPaymentId, razorpaySignature };
            ...
            setApiErrors({ keyNotValid: responseData?.somethingWentWrongErrMsg })
        }
                ...
                >
                    <div className={classnames('is-full', frontCommonStyles.layoutSpacing)}>
                        <div className='flex justify-center items-center gap-3 max-sm:mlb-4 mbe-4'>
                            ...
                            <InputLabel htmlFor='pricing-switch' className='cursor-pointer'>
                                Pay Annually (Save {paymentDetails?.discountDetail?.discount}%)
                            </InputLabel>
                        </div>
                        ...
                        {paymentDetails?.userDetail?.isExpired === null && (<InputLabel htmlFor='plan-expiray' className='flex justify-center max-sm:mlb-4 mbe-4 text-red-500'>
                                    ...
                                    <Grid item key={plan.id} xs={12} lg={4}>
                                        <Card className={classnames({ 'border-2 border-primary shadow-xl': currentPlan })}>
                                            <CardContent className='flex flex-col gap-8 p-8 pbs-12'>
                                                ...
                                                {(paymentDetails?.userDetail?.isExpired === "Y" || paymentDetails?.userDetail?.isExpired === null) && (<Button variant={'contained'} onClick={() => processPayment(plan.id)}>
                                                ...
                                                {currentPlan && (
                                                    <>
                                                        ...
                                                        {apiErrors.description === 'Authentication failed' && (<Typography variant='h6' className='text-red-500'>{responseData?.somethingWentWrongErrMsg}</Typography>)}
                                                    </>
                                                )}
                        ...
                        {apiErrors?.keyNotValid && (<Typography variant='h6' className='text-center text-red-500 mt-4'>{responseData?.somethingWentWrongErrMsg}</Typography>)}
                    </div>
                </section>
            )}
        </>
